...
public class PlanningExcelWriter {
    ...
    public static String sauvegarderPlanning(List<PlanningSoutenance> planning, String uploadPath, String annee) throws Exception {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 8));  // 9 colonnes au lieu de 10
            ...
            // En-têtes colonnes (9 colonnes au lieu de 10)
            Row headerRow = sheet.createRow(3);
            String[] headers = {"N°", "Date", "Heure", "Salle", "Encadrant", "Jury 1", "Jury 2", "Etudiant", "Filiere"};
            ...
            for (PlanningSoutenance ps : planning) {
                ...
                // Etudiant (nom complet - avec couleur par filière)
                Cell cellEtudiant = row.createCell(7);
                cellEtudiant.setCellValue(ps.getEtudiantComplet());
                short couleurFiliere = ColorUtilPlanning.getCouleurFiliere(ps.getFiliere());
                applyCellStyle(cellEtudiant, couleurFiliere, workbook, dataStyle);
                
                // Filiere (avec couleur)
                Cell cellFiliere = row.createCell(8);
