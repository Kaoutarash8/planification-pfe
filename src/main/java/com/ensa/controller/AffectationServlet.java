...
public class AffectationServlet extends HttpServlet {
    ...
    private void genererAffectation(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // Sauvegarder les fichiers de façon PERMANENTE (écrase les anciens)
        ...
        String fichierAffectation = ExcelWriter.sauvegarderAffectation(affectation, timestamp);
        String fichierStats = ExcelWriter.sauvegarderStatistiques(affectation, statistiques, timestamp);
    ...
    private void genererPlanning(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String basePath = "C:/Users/e/workspace-pfe/pfe-affectation";
        ...
        String timestamp = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
        String outputPath = basePath + "/uploads/planning_" + timestamp + ".xlsx";
        planningService.sauvegarderPlanning(outputPath);
        
        try (FileInputStream fis = new FileInputStream(outputPath);
             FileOutputStream fos = new FileOutputStream(basePath + "/uploads/planning_latest.xlsx")) {
        ...
        request.getSession().setAttribute("timestampPlanning", timestamp);
    ...
    private void genererPVs(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String basePath = getBasePathTomcat();
        ...
        // Lire les étudiants depuis le dernier upload
    ...
    private List<PlanningSoutenance> lirePlanning(String filePath) throws Exception {
        ...
        try (FileInputStream fis = new FileInputStream(filePath);
            ...
            // Les données commencent à la ligne 7 (index 6) car les 7 premières lignes sont les titres
            int startRow = 7;
            ...
            for (int i = startRow; i <= sheet.getLastRowNum(); i++) {
                ...
                // Vérifier si c'est une ligne vide ou de séparation
                String num = getCellValue(row.getCell(0));
                if (num == null || num.isEmpty()) continue;
                if (num.equals("N°") || num.equals("Date")) continue;
                
                // Lire selon les colonnes correctes
                String nom = getCellValue(row.getCell(8));      // Colonne 8 = Nom Etudiant
                String prenom = getCellValue(row.getCell(9));   // Colonne 9 = Prénom Etudiant
                ...
                ps.setEtudiantCNE(getCellValue(row.getCell(1)));   // Colonne 1 = CNE
                ps.setEtudiantNom(nom);
                ps.setEtudiantPrenom(prenom);
                ps.setDate(getCellValue(row.getCell(2)));          // Colonne 2 = Date
                ps.setHeure(getCellValue(row.getCell(3)));         // Colonne 3 = Heure
                ps.setSalle(getCellValue(row.getCell(4)));         // Colonne 4 = Salle
                ps.setEncadrant(getCellValue(row.getCell(5)));     // Colonne 5 = Encadrant
                ps.setJury1(getCellValue(row.getCell(6)));         // Colonne 6 = Jury 1
                ps.setJury2(getCellValue(row.getCell(7)));         // Colonne 7 = Jury 2
                ps.setFiliere(getCellValue(row.getCell(10)));      // Colonne 10 = Filière
    ...
    private String getBasePath() {
        ...
        if (catalinaBase != null) {
            return catalinaBase + "/wtpwebapps/pfe-affectation";
        }
        return System.getProperty("user.dir");
    }
    
    private String getBasePathTomcat() {
        String catalinaBase = System.getProperty("catalina.base");
        if (catalinaBase != null) {
            return catalinaBase + "/wtpwebapps/pfe-affectation";
        }
